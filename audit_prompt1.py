...
for fname in pipeline_files:
    ...
    accepts_df   = bool(re.search(r"def \w+\([^)]*\bdf\b[^)]*\)", text))

    if not still_global and accepts_df:
        passed(f"{fname}: df received as argument — session-aware ✓")
    elif still_global and not accepts_df:
        failed(
    ...
    elif still_global and accepts_df:
        warned(
            f"{fname}: imports global df AND has a df argument — ambiguous",
            "Confirm which one the functions actually use at runtime.",
        )
    else:
        warned(
            f"{fname}: no global df import and no df argument detected",
